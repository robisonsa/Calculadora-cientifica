-- ============================================================
-- GPS — Seed SQL completo (v2 — com ano_escolar por série)
-- Pré-requisito: rodar migration 003_ano_escolar.sql antes
...
DECLARE
  ...
  mt2ef01_id UUID; mt2ef02_id UUID; mt2ef03_id UUID; mt2ef04_id UUID;

  -- IDs das habilidades 5º EF
  lp01_id UUID; lp02_id UUID; lp03_id UUID; lp04_id UUID;
  mt01_id UUID; mt02_id UUID; mt03_id UUID; mt04_id UUID;

  -- IDs das habilidades 9º EF
...
INSERT INTO public.habilidades (codigo, descricao, disciplina, eixo, nivel_escala_saeb, ano_escolar) VALUES
  -- 2º EF — Língua Portuguesa
  ('LP2EF01','Reconhecer letras e sílabas em palavras','lp','Leitura',1,'2EF'),
  ('LP2EF02','Ler e compreender palavras e frases curtas','lp','Leitura',1,'2EF'),
  ('LP2EF03','Localizar informação em texto muito curto','lp','Leitura',2,'2EF'),
  ('LP2EF04','Identificar personagem e local em texto narrativo simples','lp','Leitura',2,'2EF'),
  -- 2º EF — Matemática
  ('MT2EF01','Contar e representar quantidades de 0 a 100','matematica','Numeros',1,'2EF'),
  ('MT2EF02','Resolver adição e subtração com números até 20','matematica','Numeros',1,'2EF'),
  ('MT2EF03','Reconhecer e nomear figuras geométricas planas','matematica','Geometria',2,'2EF'),
  ('MT2EF04','Completar sequências numéricas simples','matematica','Numeros',2,'2EF'),
  -- 5º EF — Língua Portuguesa
  ('LP01','Localizar informações explícitas em textos','lp','Leitura',2,'5EF'),
  ('LP02','Inferir o sentido de palavras ou expressões','lp','Leitura',3,'5EF'),
  ('LP03','Identificar o tema central de um texto','lp','Leitura',3,'5EF'),
  ('LP04','Reconhecer a finalidade de um texto','lp','Leitura',4,'5EF'),
  -- 5º EF — Matemática
  ('MT01','Resolver problemas com números naturais e operações básicas','matematica','Numeros',2,'5EF'),
  ('MT02','Resolver problemas com frações e decimais','matematica','Numeros',3,'5EF'),
  ('MT03','Identificar e descrever figuras geométricas planas e espaciais','matematica','Geometria',3,'5EF'),
  ('MT04','Resolver problemas com porcentagem','matematica','Numeros',4,'5EF'),
  -- 9º EF — Língua Portuguesa
  ('LP05','Interpretar texto com linguagem verbal e não verbal','lp','Leitura',4,'9EF'),
  ('LP06','Reconhecer relações de causa e consequência','lp','Leitura',5,'9EF'),
  ('LP07','Distinguir fato de opinião em texto argumentativo','lp','Leitura',6,'9EF'),
  ('LP08','Reconhecer o efeito de sentido produzido por recursos expressivos','lp','Literatura',7,'9EF'),
  -- 9º EF — Matemática
  ('MT05','Interpretar gráficos e tabelas','matematica','Estatistica',4,'9EF'),
  ('MT06','Resolver equações e inequações do 1º grau','matematica','Algebra',5,'9EF'),
  ('MT07','Aplicar o Teorema de Pitágoras em situações-problema','matematica','Geometria',6,'9EF'),
  ('MT08','Resolver problemas envolvendo funções afins e quadráticas','matematica','Algebra',7,'9EF')
ON CONFLICT (codigo) DO UPDATE SET
  ano_escolar = EXCLUDED.ano_escolar,
  descricao   = EXCLUDED.descricao;

-- Busca IDs — 2EF
SELECT id INTO lp2ef01_id FROM public.habilidades WHERE codigo = 'LP2EF01';
SELECT id INTO lp2ef02_id FROM public.habilidades WHERE codigo = 'LP2EF02';
SELECT id INTO lp2ef03_id FROM public.habilidades WHERE codigo = 'LP2EF03';
SELECT id INTO lp2ef04_id FROM public.habilidades WHERE codigo = 'LP2EF04';
SELECT id INTO mt2ef01_id FROM public.habilidades WHERE codigo = 'MT2EF01';
SELECT id INTO mt2ef02_id FROM public.habilidades WHERE codigo = 'MT2EF02';
SELECT id INTO mt2ef03_id FROM public.habilidades WHERE codigo = 'MT2EF03';
SELECT id INTO mt2ef04_id FROM public.habilidades WHERE codigo = 'MT2EF04';
-- Busca IDs — 5EF
...
INSERT INTO public.questoes (habilidade_id,enunciado,alternativa_a,alternativa_b,alternativa_c,alternativa_d,gabarito,nivel_dificuldade,fonte) VALUES
  ...
  -- LP2EF01 — Letras e sílabas
  (lp2ef01_id,'Quantas sílabas tem a palavra BORBOLETA?','2','3','4','5','c',1,'elaborada'),
  (lp2ef01_id,'Qual sílaba completa a palavra CA___ para dar o nome do lugar onde moramos?','BO','MA','SA','TA','c',1,'elaborada'),
  (lp2ef01_id,'Qual palavra tem apenas UMA sílaba?','CASA','PÁ','GATO','BOLO','b',1,'elaborada'),
  (lp2ef01_id,'Qual palavra começa com a letra M?','PATO','BOLA','MATO','DADO','c',1,'elaborada'),
  (lp2ef01_id,'Quantas letras tem a palavra ESCOLA?','4','5','6','7','c',1,'elaborada'),

  -- LP2EF02 — Palavras e frases curtas
  (lp2ef02_id,'O que a frase "a porta está aberta" significa?','A porta está fechada','A porta não tem chave','A porta permite entrada','A porta é velha','c',1,'elaborada'),
  (lp2ef02_id,'Qual frase está correta?','O gato late','O cachorro mia','O gato mia','O pato nada no céu','c',1,'elaborada'),
  (lp2ef02_id,'Leia: "Ana tem um cachorro." Quem tem o cachorro?','O cachorro','Ana','A escola','O jardim','b',1,'elaborada'),
  (lp2ef02_id,'O que quer dizer "o céu está nublado"?','Está cheio de estrelas','Está com nuvens','Está com muito sol','Está de noite','b',1,'elaborada'),
  (lp2ef02_id,'Qual palavra significa o OPOSTO de GRANDE?','Alto','Forte','Pequeno','Bonito','c',1,'elaborada'),

  -- LP2EF03 — Localizar informação em texto curto
  (lp2ef03_id,'Leia: "O João tem 7 anos. Ele gosta de brincar de bola." Quantos anos tem João?','5','6','7','8','c',1,'elaborada'),
  (lp2ef03_id,'Leia: "A escola abre às 7h e fecha às 17h." Que horas a escola abre?','6h','7h','8h','17h','b',1,'elaborada'),
  (lp2ef03_id,'Leia: "Maria tem um gato chamado Bolinha." Como se chama o gato?','Maria','Gatinho','Bolinha','Bola','c',1,'elaborada'),
  (lp2ef03_id,'Leia: "Pedro mora em Aracaju." Onde mora Pedro?','Salvador','Recife','Fortaleza','Aracaju','d',1,'elaborada'),
  (lp2ef03_id,'Leia: "O ônibus parte às 8h da manhã." Quando o ônibus parte?','À tarde','À noite','Às 8h da manhã','Ao meio-dia','c',1,'elaborada'),

  -- LP2EF04 — Personagem e local em narrativa simples
  (lp2ef04_id,'Leia: "A raposa entrou na floresta para encontrar seus filhotes." Onde a raposa foi?','Na cidade','Na floresta','Na fazenda','No rio','b',1,'elaborada'),
  (lp2ef04_id,'No conto dos Três Porquinhos, quem é o personagem malvado?','Os porquinhos','O lobo mau','O caçador','O coelho','b',1,'elaborada'),
  (lp2ef04_id,'No conto da Chapeuzinho Vermelho, onde mora a vovó?','No castelo','Na floresta','Na cidade','Na escola','b',1,'elaborada'),
  (lp2ef04_id,'Leia: "A tartaruga e a lebre fizeram uma corrida." Quais são os personagens?','O leão e o elefante','A tartaruga e a lebre','O coelho e o gato','O rato e o gato','b',1,'elaborada'),
  (lp2ef04_id,'Leia: "O menino nadava no rio com os amigos." Onde acontece a história?','No mar','Na piscina','No lago','No rio','d',1,'elaborada'),

  -- MT2EF01 — Números até 100
  (mt2ef01_id,'Qual número vem DEPOIS do 19?','18','20','21','90','b',1,'elaborada'),
  (mt2ef01_id,'Qual é o MAIOR número: 45, 54, 44 ou 55?','45','44','54','55','d',1,'elaborada'),
  (mt2ef01_id,'Quais números estão em ordem CRESCENTE?','30, 20, 10','10, 20, 30','20, 10, 30','30, 10, 20','b',1,'elaborada'),
  (mt2ef01_id,'Quantas DEZENAS tem o número 70?','7','70','0','17','a',1,'elaborada'),
  (mt2ef01_id,'Qual número representa "cinco dezenas e três unidades"?','35','53','305','503','b',1,'elaborada'),

  -- MT2EF02 — Adição e subtração até 20
  (mt2ef02_id,'Quanto é 8 + 5?','12','13','14','15','b',1,'elaborada'),
  (mt2ef02_id,'Maria tinha 10 balas e deu 4 para um amigo. Quantas ficaram?','14','4','6','8','c',1,'elaborada'),
  (mt2ef02_id,'Quanto é 15 - 7?','6','7','8','9','c',1,'elaborada'),
  (mt2ef02_id,'Pedro tem 9 figurinhas e ganhou mais 6. Quantas ele tem agora?','3','14','15','16','c',1,'elaborada'),
  (mt2ef02_id,'Numa sala há 12 meninas e 8 meninos. Quantas crianças há no total?','4','18','20','22','c',1,'elaborada'),

  -- MT2EF03 — Figuras geométricas planas
  (mt2ef03_id,'Qual figura geométrica tem 4 lados IGUAIS?','Triângulo','Círculo','Quadrado','Retângulo','c',1,'elaborada'),
  (mt2ef03_id,'Quantos lados tem um triângulo?','2','3','4','5','b',1,'elaborada'),
  (mt2ef03_id,'Qual figura NÃO tem lados (é completamente redonda)?','Quadrado','Retângulo','Triângulo','Círculo','d',1,'elaborada'),
  (mt2ef03_id,'Qual objeto tem formato de círculo?','Uma caixa','Uma folha de papel','Uma moeda','Um livro','c',1,'elaborada'),
  (mt2ef03_id,'Um retângulo tem quantos lados?','3','4','5','6','b',1,'elaborada'),

  -- MT2EF04 — Sequências numéricas
  (mt2ef04_id,'Complete a sequência: 2, 4, 6, ___, 10','7','8','9','11','b',1,'elaborada'),
  (mt2ef04_id,'Complete: 5, 10, 15, ___, 25','16','18','20','22','c',1,'elaborada'),
  (mt2ef04_id,'Qual é o próximo número: 1, 3, 5, 7, ___?','8','9','10','11','b',1,'elaborada'),
  (mt2ef04_id,'Complete: 100, 90, 80, ___, 60','85','75','70','65','c',1,'elaborada'),
  (mt2ef04_id,'Na sequência 3, 6, 9, 12, qual é o padrão?','Somar 2','Somar 3','Somar 4','Multiplicar por 2','b',1,'elaborada'),
...
FROM auth.users au WHERE au.email = 'robison@gps.demo'
...
SELECT gen_random_uuid(), au.id,
  json_build_object('sub', au.id::text, 'email', au.email)::jsonb,
  'email', au.email, NOW(), NOW(), NOW()
FROM auth.users au WHERE au.email = 'gestora@gps.demo'
ON CONFLICT (provider, provider_id) DO NOTHING;

-- João (estudante — 9º EF)
IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = 'joao@gps.demo') THEN
  INSERT INTO auth.users (
    ...
    crypt('gps2026', gen_salt('bf')), NOW(),
    '{"provider":"email","providers":["email"]}',
    '{"nome_completo":"João Silva","role":"estudante","ano_escolar":"9EF"}',
    NOW(), NOW(), '', '', '', ''
  );
END IF;
INSERT INTO auth.identities (id, user_id, identity_data, provider, provider_id, created_at, updated_at, last_sign_in_at)
SELECT gen_random_uuid(), au.id,
  json_build_object('sub', au.id::text, 'email', au.email)::jsonb,
  'email', au.email, NOW(), NOW(), NOW()
FROM auth.users au WHERE au.email = 'joao@gps.demo'
ON CONFLICT (provider, provider_id) DO NOTHING;

-- Maria (estudante — 5º EF)
IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = 'maria@gps.demo') THEN
  INSERT INTO auth.users (
    ...
    crypt('gps2026', gen_salt('bf')), NOW(),
    '{"provider":"email","providers":["email"]}',
    '{"nome_completo":"Maria Oliveira","role":"estudante","ano_escolar":"5EF"}',
    NOW(), NOW(), '', '', '', ''
  );
END IF;
INSERT INTO auth.identities (id, user_id, identity_data, provider, provider_id, created_at, updated_at, last_sign_in_at)
SELECT gen_random_uuid(), au.id,
  json_build_object('sub', au.id::text, 'email', au.email)::jsonb,
  'email', au.email, NOW(), NOW(), NOW()
FROM auth.users au WHERE au.email = 'maria@gps.demo'
ON CONFLICT (provider, provider_id) DO NOTHING;

-- Pedro (estudante — 2º EF)
IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = 'pedro@gps.demo') THEN
  INSERT INTO auth.users (
    ...
    crypt('gps2026', gen_salt('bf')), NOW(),
    '{"provider":"email","providers":["email"]}',
    '{"nome_completo":"Pedro Santos","role":"estudante","ano_escolar":"2EF"}',
    NOW(), NOW(), '', '', '', ''
  );
END IF;
INSERT INTO auth.identities (id, user_id, identity_data, provider, provider_id, created_at, updated_at, last_sign_in_at)
SELECT gen_random_uuid(), au.id,
  json_build_object('sub', au.id::text, 'email', au.email)::jsonb,
  'email', au.email, NOW(), NOW(), NOW()
FROM auth.users au WHERE au.email = 'pedro@gps.demo'
ON CONFLICT (provider, provider_id) DO NOTHING;

RAISE NOTICE 'Usuários auth OK';

-- ===== 6. AJUSTAR TURMA E ANO ESCOLAR DOS ESTUDANTES =====
...
INSERT INTO public.sequencias_didaticas (habilidade_id, titulo, objetivo, materiais, etapas, tempo_estimado_minutos, ano_escolar) VALUES
  (lp01_id,'LP01: Localizar informações explícitas em textos','Desenvolver a habilidade de localizar informações explícitas conforme a Matriz SAEB.','Textos impressos ou projetados, lousa, atividades de fixação.','**Etapa 1 (10 min)** Apresente um texto curto e peça que identifiquem uma informação específica.\n\n**Etapa 2 (15 min)** Explique informações explícitas vs implícitas com exemplos do cotidiano.\n\n**Etapa 3 (15 min)** Resolva exercícios coletivamente, destacando palavras-chave.\n\n**Etapa 4 (10 min)** Estudantes resolvem 2 questões individualmente. Corrija.',50,'5EF'),
  (lp02_id,'LP02: Inferir sentido de palavras ou expressões','Desenvolver a habilidade de inferência a partir do contexto textual.','Textos impressos ou projetados, lousa, atividades de fixação.','**Etapa 1 (10 min)** Apresente expressões idiomáticas e discuta seus sentidos.\n\n**Etapa 2 (15 min)** Explique inferência pelo contexto. Mostre como o texto revela o sentido.\n\n**Etapa 3 (15 min)** Trabalhe textos com metáforas coletivamente.\n\n**Etapa 4 (10 min)** Estudantes resolvem questões de inferência. Corrija.',50,'5EF'),
  (lp03_id,'LP03: Identificar o tema central de um texto','Diferenciar tema de assunto e identificar a ideia central.','Textos impressos ou projetados, lousa.','**Etapa 1 (10 min)** Apresente dois textos sobre o mesmo tema. Discuta o que têm em comum.\n\n**Etapa 2 (15 min)** Explique tema vs assunto com mapas mentais.\n\n**Etapa 3 (15 min)** Leia um texto coletivamente e construa a identificação do tema.\n\n**Etapa 4 (10 min)** Estudantes identificam o tema de textos curtos.',50,'5EF'),
  (lp04_id,'LP04: Reconhecer a finalidade de um texto','Identificar para que serve cada tipo de texto: informar, instruir, persuadir, entreter.','Diferentes gêneros textuais impressos: bula, charge, notícia, poema.','**Etapa 1 (10 min)** Mostre gêneros textuais variados. Pergunte para que serve cada um.\n\n**Etapa 2 (15 min)** Explique os tipos de finalidade com exemplos.\n\n**Etapa 3 (15 min)** Identifique a finalidade de 3 textos diferentes coletivamente.\n\n**Etapa 4 (10 min)** Estudantes classificam textos por finalidade.',50,'5EF'),
  (lp05_id,'LP05: Interpretar texto verbal e não verbal','Integrar linguagem verbal e não verbal na produção de sentido.','Charges, tirinhas, propagandas impressas ou projetadas.','**Etapa 1 (10 min)** Mostre charges e tirinhas. Pergunte: o que o texto verbal diz? E o não verbal?\n\n**Etapa 2 (15 min)** Explique as duas linguagens e como interagem.\n\n**Etapa 3 (15 min)** Analisem uma propaganda e uma charge coletivamente.\n\n**Etapa 4 (10 min)** Estudantes interpretam textos multimodais.',50,'9EF'),
  (lp06_id,'LP06: Relações de causa e consequência','Reconhecer conectivos e relações causais em textos.','Textos jornalísticos, atividades com conectivos.','**Etapa 1 (10 min)** Apresente situações cotidianas com causa e efeito.\n\n**Etapa 2 (15 min)** Explique conectivos: porque, por isso, portanto, logo.\n\n**Etapa 3 (15 min)** Identifique relações em textos jornalísticos coletivamente.\n\n**Etapa 4 (10 min)** Estudantes completam frases com conectivos.',50,'9EF'),
  (lp07_id,'LP07: Fato e opinião em texto argumentativo','Distinguir afirmações verificáveis de posicionamentos subjetivos.','Artigos de opinião, editoriais.','**Etapa 1 (10 min)** Apresente afirmações para classificar como fato ou opinião.\n\n**Etapa 2 (15 min)** Explique características de fatos verificáveis vs opiniões.\n\n**Etapa 3 (15 min)** Analise um artigo de opinião coletivamente.\n\n**Etapa 4 (10 min)** Estudantes classificam afirmações.',50,'9EF'),
  (lp08_id,'LP08: Recursos expressivos e efeitos de sentido','Identificar figuras de linguagem e seus efeitos no texto.','Poemas, trechos literários, textos publicitários.','**Etapa 1 (10 min)** Apresente poemas com figuras de linguagem. O que chamou atenção?\n\n**Etapa 2 (15 min)** Explique metáfora, símile, hipérbole, ironia e personificação.\n\n**Etapa 3 (15 min)** Identifiquem recursos em textos coletivamente.\n\n**Etapa 4 (10 min)** Estudantes identificam e nomeiam figuras de linguagem.',50,'9EF'),
  (mt01_id,'MT01: Números naturais e operações básicas','Resolver situações-problema com as quatro operações.','Situações-problema do cotidiano, lousa.','**Etapa 1 (10 min)** Apresente situações-problema cotidianas.\n\n**Etapa 2 (15 min)** Revise as quatro operações e a interpretação de problemas.\n\n**Etapa 3 (15 min)** Resolva 3 problemas coletivamente.\n\n**Etapa 4 (10 min)** Estudantes resolvem 2 problemas individualmente.',50,'5EF'),
  (mt02_id,'MT02: Frações e números decimais','Operar com frações e decimais em situações contextualizadas.','Materiais concretos (pizza, chocolate), lousa.','**Etapa 1 (10 min)** Use materiais concretos para representar frações.\n\n**Etapa 2 (15 min)** Explique frações equivalentes e decimais.\n\n**Etapa 3 (15 min)** Resolva problemas coletivamente.\n\n**Etapa 4 (10 min)** Estudantes resolvem exercícios.',50,'5EF'),
  (mt03_id,'MT03: Figuras geométricas planas e espaciais','Identificar e classificar figuras geométricas.','Objetos do cotidiano, modelos geométricos, imagens.','**Etapa 1 (10 min)** Mostre objetos e peça que identifiquem as formas.\n\n**Etapa 2 (15 min)** Caracterize polígonos, poliedros e corpos redondos.\n\n**Etapa 3 (15 min)** Classifiquem figuras e calculem perímetros simples.\n\n**Etapa 4 (10 min)** Estudantes resolvem questões de identificação.',50,'5EF'),
  (mt04_id,'MT04: Porcentagem','Aplicar porcentagem em situações reais: desconto, aumento, taxa.','Situações reais de loja, banco e pesquisas.','**Etapa 1 (10 min)** Mostre situações: desconto em loja, juros. Como calculariam?\n\n**Etapa 2 (15 min)** Explique porcentagem como fração de 100.\n\n**Etapa 3 (15 min)** Resolva problemas de desconto e aumento coletivamente.\n\n**Etapa 4 (10 min)** Estudantes resolvem 2 problemas.',50,'5EF'),
  (mt05_id,'MT05: Gráficos e tabelas','Ler, interpretar e extrair informações de gráficos e tabelas.','Gráficos de jornais/revistas, calculadora.','**Etapa 1 (10 min)** Mostre gráficos reais. Que informações podem ser extraídas?\n\n**Etapa 2 (15 min)** Explique tipos de gráficos e como calcular média, moda e mediana.\n\n**Etapa 3 (15 min)** Interprete uma tabela e um gráfico coletivamente.\n\n**Etapa 4 (10 min)** Estudantes respondem questões sobre um gráfico.',50,'9EF'),
  (mt06_id,'MT06: Equações e inequações do 1º grau','Resolver equações e inequações com uma incógnita.','Balança (metáfora), exercícios graduais.','**Etapa 1 (10 min)** Apresente uma balança como metáfora da equação.\n\n**Etapa 2 (15 min)** Explique o método de resolução com exemplos graduais.\n\n**Etapa 3 (15 min)** Resolva equações e inequações coletivamente.\n\n**Etapa 4 (10 min)** Estudantes resolvem equações individualmente.',50,'9EF'),
  (mt07_id,'MT07: Teorema de Pitágoras','Aplicar o Teorema de Pitágoras em situações-problema.','Papel quadriculado, situações práticas (construção, mapa).','**Etapa 1 (10 min)** Mostre situações onde Pitágoras é usado na prática.\n\n**Etapa 2 (15 min)** Enuncie o teorema (a² + b² = c²). Demonstre visualmente.\n\n**Etapa 3 (15 min)** Resolva 3 problemas aplicados coletivamente.\n\n**Etapa 4 (10 min)** Estudantes resolvem 2 problemas.',50,'9EF'),
  (mt08_id,'MT08: Funções afins e quadráticas','Compreender e aplicar funções em situações cotidianas.','Situações cotidianas, gráficos, calculadora.','**Etapa 1 (10 min)** Mostre situações modeladas por funções: preço×quantidade, queda livre.\n\n**Etapa 2 (15 min)** Explique função afim e quadrática. Construa gráficos.\n\n**Etapa 3 (15 min)** Calcule valores e interprete gráficos coletivamente.\n\n**Etapa 4 (10 min)** Estudantes resolvem exercícios com funções.',50,'9EF');

RAISE NOTICE 'Sequências OK';
RAISE NOTICE '✅ Seed concluído! Contas demo: joao@gps.demo (9EF), maria@gps.demo (5EF), pedro@gps.demo (2EF) | senha: gps2026';
